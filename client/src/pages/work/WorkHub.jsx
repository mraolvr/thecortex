    ...
    <div className="min-h-screen bg-neutral-950 p-10">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-10">
          <div className="bg-neutral-900 rounded-xl p-4 flex flex-col items-center shadow-md">
          ...
          <div className="bg-neutral-900 rounded-xl p-4 flex flex-col items-center shadow-md">
            <span className="text-2xl font-bold text-cyan-400">{totalProjects}</span>
            <span className="text-xs text-neutral-400 mt-1">Total Projects</span>
          </div>
          <div className="bg-neutral-900 rounded-xl p-4 flex flex-col items-center shadow-md">
            <span className="text-2xl font-bold text-yellow-400">{inProgressGoals}</span>
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 md:col-span-4 flex flex-col gap-6">
            <div className="bg-neutral-900 rounded-xl shadow-md p-6 flex flex-col h-full">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-2xl font-semibold text-white">Tasks</h2>
                <button
                  onClick={() => setShowAddTaskModal(true)}
                  className="bg-primary hover:bg-primary-light text-white px-3 py-2 rounded-lg transition-colors hidden md:block"
                >
              ...
              <div className="flex gap-2 mb-4">
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full bg-primary  pl-10 pr-4 py-2 rounded-lg border border-neutral-700 focus:outline-none focus:border-primary text-blue"
                  />
                  <Search className="absolute left-3 top-2.5 w-5 h-5 text-black-400" />
                </div>
                <select
                  value={taskFilter}
                  onChange={e => setTaskFilter(e.target.value)}
                  className="bg-primary text-white-900 rounded-lg px-3 py-2 border border-neutral-700"
                >
                ...
                {filteredTasks.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-3">
                    {filteredTasks.map(task => (
                      <div
                        key={task.id}
                        className={`flex items-center justify-between p-3 bg-gradient-to-br from-blue-600/80 dark:to-blue-900/80 border-l-4 border-blue-400 dark:border-blue-500 rounded-lg shadow ${task.status === 'done' ? 'opacity-60' : ''}`}
                      >
                        <div className="flex items-center gap-3">
                          <button
                            onClick={e => handleToggleTask(task, e)}
                            className="focus:outline-none focus:ring-2 focus:ring-primary rounded"
                            title={task.status === 'done' ? 'Mark as To Do' : 'Complete'}
                            disabled={completingTaskId === task.id}
                          >
                            {completingTaskId === task.id ? (
                              <span className="w-5 h-5 animate-spin border-2 border-primary border-t-transparent rounded-full inline-block"></span>
                            ) : task.status === 'done' ? (
                              <Check className="w-5 h-5 text-green-400" />
                            ) : (
                              <span className="inline-block w-5 h-5 border-2 border-neutral-400 rounded-full"></span>
                            )}
                          </button>
                          {editingTaskId === task.id ? (
                            <form onSubmit={e => { e.preventDefault(); handleSaveEditTask(task.id); }} className="flex flex-col gap-2">
                              <input
                                type="text"
                                value={editTaskForm.title}
                                onChange={e => setEditTaskForm({ ...editTaskForm, title: e.target.value })}
                                className="bg-neutral-900 px-2 py-1 rounded text-white"
                                required
                              />
                              <textarea
                              ...
                              <div className="flex gap-2 mt-2">
                                <button type="submit" className="bg-primary text-black rounded px-3 py-1">Save</button>
                                <button type="button" onClick={() => setEditingTaskId(null)} className="bg-neutral-700 text-white rounded px-3 py-1">Cancel</button>
                              </div>
                            </form>
                          ) : (
                            <div>
                              <h3 className="font-medium text-white truncate max-w-[180px]">{task.title}</h3>
      ...
      {showAddTaskModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-neutral-900 rounded-xl p-6 w-full max-w-md border border-neutral-800 shadow-2xl" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            >
              <input
                ...
                className="w-full px-3 py-2 bg-neutral-800 border border-neutral-700 rounded-lg text-white focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
                autoFocus
                required
              />
              <textarea
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="px-3 py-2 bg-neutral-800 rounded-lg text-neutral-200"
                >
                  <option value="low">Low Priority</option>
                  <option value="medium">Medium Priority</option>
                  <option value="high">High Priority</option>
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-neutral-800 rounded-lg text-neutral-200"
                />
              </div>
              <select
                value={newTask.project_id}
                onChange={e => setNewTask({ ...newTask, project_id: e.target.value })}
                className="w-full px-3 py-2 bg-neutral-800 rounded-lg text-neutral-200"
              >
                <option value="">No Project</option>
                {projects.map(project => (
                  <option key={project.id} value={project.id}>{project.name}</option>
